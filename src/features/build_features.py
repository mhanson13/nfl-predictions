...
def _load_sr_pbp_subset() -> pd.DataFrame:
    ...
    if not pbp_path.exists():
        return pd.DataFrame()
    try:
        return pd.read_parquet(pbp_path, columns=PBP_SR_COLUMNS)
    except Exception as exc:
        print(f"[sr_substitute] subset read failed ({exc}); loading full play-by-play")
        try:
            return pd.read_parquet(pbp_path, columns=PBP_SR_COLUMNS, engine="fastparquet")
        except Exception:
            pass
        try:
            return pd.read_parquet(pbp_path)
        except Exception as fallback_exc:
            print(f"[sr_substitute] unable to read play-by-play: {fallback_exc}")
            try:
                return pd.read_parquet(pbp_path, engine="fastparquet")
            except Exception:
                return pd.DataFrame()
